...
// QueryUpstreamProxy chains the current query on the "upstream_proxy" edge.
func (_q *ProxyQuery) QueryUpstreamProxy() *ProxyQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, false, proxy.UpstreamProxyTable, proxy.UpstreamProxyColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *ProxyQuery) Clone() *ProxyQuery {
	...
	return &ProxyQuery{
		...
		withUpstreamProxy: _q.withUpstreamProxy.Clone(),
...
// WithUpstreamProxy tells the query-builder to eager-load the nodes that are connected to
// the "upstream_proxy" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ProxyQuery) WithUpstreamProxy(opts ...func(*ProxyQuery)) *ProxyQuery {
	query := (&ProxyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUpstreamProxy = query
	return _q
}
...
func (_q *ProxyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Proxy, error) {
	...
	if query := _q.withUpstreamProxy; query != nil {
		if err := _q.loadUpstreamProxy(ctx, query, nodes, nil,
			func(n *Proxy, e *Proxy) { n.Edges.UpstreamProxy = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProxyQuery) loadUpstreamProxy(ctx context.Context, query *ProxyQuery, nodes []*Proxy, init func(*Proxy), assign func(*Proxy, *Proxy)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Proxy)
	for i := range nodes {
		if nodes[i].UpstreamProxyID == nil {
			continue
		}
		fk := *nodes[i].UpstreamProxyID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "upstream_proxy_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ProxyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUpstreamProxy != nil {
			_spec.Node.AddColumnOnce(proxy.FieldUpstreamProxyID)
		}
	}
