...
const apiKeyAuthSnapshotVersion = 12 // v12: reload snapshots for models_list_config and Kiro cache emulation fields
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	if groupForSnapshot != nil {
		snapshot.Group = &APIKeyAuthGroupSnapshot{
			...
			AllowCrossPlatformFallback:      groupForSnapshot.AllowCrossPlatformFallback,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	if snapshot.Group != nil {
		apiKey.Group = &Group{
			...
			AllowCrossPlatformFallback:      snapshot.Group.AllowCrossPlatformFallback,
